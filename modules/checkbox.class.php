<?php
	...
	class AUF_CHECKBOX extends AUF_FILTER_ELEMENTS {
		...
		function render_element( $settings, $modul = array(), $filter = array(), $index = 0, $current_value ) {
			...
			$html = '<div class="' . auf_get_element_classes( $this->ID, 'select' ) . '" id="' . auf_get_the_elements_id() . '">';
			...
			foreach ( $options as $option ) {
				$checked = '';
				if ( ! is_array( $option ) ) {
					if ( is_array( $current_value ) && in_array( $option, $current_value ) ) {
						$checked = 'checked="checked"';
					}

					$html .= '<input ' . $checked . ' type="checkbox" id="' . auf_get_the_elements_id() . '-' . sanitize_key( $option ) . '" name="' . auf_get_the_elements_name() . '[]" value="' . esc_attr( $option ) . '"><label for="' . auf_get_the_elements_id() . '-' . sanitize_key( $option ) . '">' . $option . '</label>';
				} else {	
					if ( is_array( $current_value ) && in_array( $option['value'], $current_value ) ) {
						$checked = 'checked="checked"';	
					}			

					$html .= '<input ' . $checked . ' type="checkbox" id="' . auf_get_the_elements_id() . '-' . sanitize_key( $option['value'] ) . '" name="' . auf_get_the_elements_name() . '[]" value="' . esc_attr( $option['value'] ) . '"><label for="' . auf_get_the_elements_id() . '-' . sanitize_key( $option['value'] ) . '">' . $option['label'] . '</label>'; 
				}
			}
			$html .= '</div>';
			return $html;
